...
void TextEditorMenu(const char * name) {
	...
	if (editors.find(s_name) == editors.end()) {

		editors[name] = new TextEditor();
		editors[name]->SetLanguageDefinition(TextEditor::LanguageDefinition::Lua());
		//std::cout << "Testando" << std::endl;
	}
	TextEditor* editor = editors[name];
...
void RenderTextEditor(const char *name) {
	...
	/*char *result = (char*)malloc(strlen("TextEditor_") + strlen(name) + 1);
	strcat(result, "TextEditor_");
	strcat(result,  name);*/
	editors[name]->Render(name);
}
